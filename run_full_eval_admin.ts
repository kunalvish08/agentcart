...
// Using the admin key from env if available, otherwise anon might fail RLS
...
async function run() {
  ...
  while (totalProcessed < target) {
    ...
    try {
      // processEvaluationBatch usually expects a context or uses the default client
      // Let's ensure it has what it needs.
